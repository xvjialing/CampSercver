...
<body>
	...
	<div class="cf">
		<div class="fl">
            <a class="btn" href="<?php echo U('add',array('agentID'=>I('get.agentID',0)));?>">新 增</a>
           <button class="btn ajax-post confirm" url="<?php echo U('Ly/changeStatus',array('method'=>'del'));?>" target-form="ids">删 除</button>
        </div>

        <!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="agentName" class="search-input" value="<?php echo I('agentName');?>" placeholder="请输入用户昵称或者ID">
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('uindex');?>"><i class="btn-search"></i></a>
			</div>
		</div>
    </div>
    <!-- 数据列表 -->
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            <td><input class="ids" type="checkbox" name="agentid[]" value="<?php echo ($vo["agentid"]); ?>" /></td>
			<td><?php echo ($vo["agentID"]); ?> </td>
			<td><?php echo ($vo["agentName"]); ?></td>
			<td><?php echo ($vo["agentTel"]); ?></td>
			<td><?php echo ($vo["agentEmail"]); ?></td>
			<td><?php echo ($vo["realName"]); ?></td>
			<td><?php echo ($vo["agentAddress"]); ?></td>
			<td><span><?php echo ($vo["addTime"]); ?></span></td>
			<td>
				<a title="编辑" href="<?php echo U('update?agentID='.$vo['agentID']);?>">编辑</a>
                <a class="confirm ajax-get" title="删除" href="<?php echo U('del?agentID='.$vo['agentID']);?>">删除</a>
            </td>
...
<!--     <script type="text/javascript" src="/onethink1/Public/Admin/js/common.js"></script> -->
	...
	<script src="/onethink1/Public/static/thinkbox/jquery.thinkbox.js"></script>
    ...
    //导航高亮
    highlight_subnav('<?php echo U('Ly/index');?>');
	</script>
