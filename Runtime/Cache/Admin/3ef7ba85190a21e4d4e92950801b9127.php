...
<body>
    ...
    <div class="cf">
        <a class="btn" href="<?php echo U('add',array('gameID'=>I('get.gameID',0)));?>">新 增</a>
        <button class="btn ajax-post confirm" url="<?php echo U('gdel');?>" target-form="ids">删 除</button>
       
        <!-- 高级搜索 -->
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="title" class="search-input" value="<?php echo I('gameName');?>" placeholder="请输入游戏名称">
                <a class="sch-btn" href="javascript:;" id="search" url="/onethink1/index.php?s=/Admin/Game/detail/campID/14.html"><i class="btn-search"></i></a>
            </div>
        </div>
    </div>
    ...
    <div class="data-table table-striped">
        <form class="ids">
            <table>
                ...
                <tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$game): $mod = ($i % 2 );++$i;?><tr>
                        <td><input class="ids row-selected" type="checkbox" name="bivouacID[]" value="<?php echo ($game["gameID"]); ?>"></td>
                        <td><?php echo ($game["gameID"]); ?></td>
                        <td>
                            <!-- <a href="<?php echo U('glist2?campID='.$camp['campID']);?>"></a> --><?php echo ($game["gameName"]); ?>
                        </td>
                        <td><?php echo ($game["gameType"]); ?></td>
                        <td><?php echo ($game["gameGrade"]); ?></td>
                        <td><?php echo ($game["gameHot"]); ?></td>
                        <td>
                            <!-- <a href="<?php echo U('toogleSta',array('bivouacID'=>$camp['bivouacID'],'value'=>abs($camp['Status']-1)));?>" class="ajax-get"> </a>-->
                            <?php echo ($game["Status_text"]); ?>
                            
                        </td>                    
                        <td>
                            <a title="编辑" href="<?php echo U('edit?gameID='.$game['gameID']);?>">编辑</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('delete?campID='.$camp['campID']);?>">删除</a>
                        </td>
