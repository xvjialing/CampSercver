...
<body>
    ...
    <div class="cf">
        ...
        <button class="btn ajax-post confirm" url="<?php echo U('cdelete');?>" target-form="ids">删 除</button>
       
        <!-- 高级搜索 -->
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="title" class="search-input" value="<?php echo I('orderID');?>" placeholder="请输入订单编号">
                <a class="sch-btn" href="javascript:;" id="search" url="/onethink1/index.php?s=/Admin/Sale/detail/orderID/1.html"><i class="btn-search"></i></a>
            </div>
        </div>
    </div>
    ...
    <div class="data-table table-striped">
        <form class="ids">
            <table>
                ...
                <tbody>
				<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$detail): $mod = ($i % 2 );++$i;?><tr>
                        <td><input class="ids row-selected" type="checkbox" name="orderID[]" value="<?php echo ($detail["ID"]); ?>"></td>
                        <td><?php echo ($detail["ID"]); ?></td>
                        <td><?php echo ($detail["username"]); ?></td>
                        <td><?php echo ($detail["mobile"]); ?></td>
                        <td><?php echo ($detail["userGrade"]); ?></td>
                        <td><?php echo ($detail["presetTime"]); ?></td>
                        <td><?php echo ($detail["Status_text"]); ?></td>                                         
                        <td>
                            <a title="查看详情" href="<?php echo U('detail?orderID='.$order['orderID']);?>">查看详情</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('delete?orderID='.$order['orderID']);?>">退款</a>
                        </td>
