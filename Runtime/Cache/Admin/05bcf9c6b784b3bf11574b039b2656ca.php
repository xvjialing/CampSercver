...
<body>
    ...
    <div class="cf">
        <a class="btn" href="<?php echo U('cadd',array('bivouacID'=>I('get.bivouacID',0)));?>">新 增</a>
        <button class="btn ajax-post confirm" url="<?php echo U('cdelete?bivouacID='.$bivouac['bivouacID']);?>" target-form="ids">删 除</button>
       
        <!-- 高级搜索 -->
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="title" class="search-input" value="<?php echo I('title');?>" placeholder="请输入菜单名称">
                <a class="sch-btn" href="javascript:;" id="search" url="/onethink1/index.php?s=/Admin/AgCamp/clist.html"><i class="btn-search"></i></a>
            </div>
        </div>
    </div>
    ...
    <div class="data-table table-striped">
        <form class="ids">
            <table>
                ...
                <tbody>
				<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$camp): $mod = ($i % 2 );++$i;?><tr>
                        <td><input class="ids row-selected" type="checkbox" name="bivouacID[]" value="<?php echo ($camp["bivouacID"]); ?>"></td>
                        <td><?php echo ($camp["bivouacID"]); ?></td>
                        <td>
                            <a href="<?php echo U('camp?bivouacID='.$camp['bivouacID']);?>"><?php echo ($camp["bivName"]); ?></a>
                        </td>
                        <td><?php echo ($camp["bivAdd"]); ?></td>
                        <td><?php echo ($camp["count"]); ?></td>
                        <td><?php echo ($camp["bivVisit"]); ?></td>
                        <td><?php echo ($camp["bivEva"]); ?></td>
                        <td>
                            <!-- <a href="<?php echo U('toogleSta',array('bivouacID'=>$camp['bivouacID'],'value'=>abs($camp['Status']-1)));?>" class="ajax-get"> -->
                            <?php echo ($camp["Status_text"]); ?>
                            </a>
                        </td>                    
                        <td>
                            <a title="编辑" href="<?php echo U('cupdate?bivouacID='.$camp['bivouacID']);?>">编辑</a>
                            <a class="confirm ajax-get" title="删除" href="<?php echo U('cdelete?bivouacID='.$camp['bivouacID']);?>">删除</a>
                        </td>
